am Beispiel des Stellverhaltens mit der ÜF $G(s) = G_1 G_2 $

\section{Betrachtung der Strecken ÜF}
\subsection{Polynomform der ÜF}
  \[ G(s) = \frac{b_ms^m + b_{m-1}s^{m-1} + ... + b_0}{a_ns^n + a_{n-1}s^{n-1} + ... + a_0} = \frac{Z(s)}{N(s)} \], wobei
 \begin{itemize}
  \item  $ a_i, b_j \in \mathbb{R} \text{ mit } a_n \not= 0 \wedge b_m \not= 0$ 
  \item m = grad $Z(s)$: Zählergrad
  \item n = grad $N(s)$: Nennergrad bzw. Systemordnung
  \item $\delta = m-n$: Differenzgrad bzw. Relativgrad des Systems
  \end{itemize}
  \[  	\left.\begin{matrix}
	    \delta > 0 \text{ verzögerndes} \\
	    \delta = 0 \text{ nicht verzögerndes} \\
	    \delta < 0 \text{ differenzierendes} \\
	\end{matrix}\right\}
	\text{Verhalten des Systems}
  \]
 
Da im allgemeinen $n \geq m$, d.h. $\delta \geq$  ist $\delta$ ein Maß für die verzögernde Wirkung eines Systems 
(siehe \bb{13})

\subsection{Pol- Nullstellenform der ÜF}
Sind $n_1,...,n_m$ bzw. $p_i,...,p_n$ die (reellen oder konjugiert komplexen) Wurzeln von $Z(s)=0$ bzw. $N(s)=0$ so folgt:
 ...
 Dabei gilt: \begin{itemize}
		    ...
		    Dieses $n_j$ ist eine NS $s_0$ der ÜF
	      \item $ \left| G(s=p_i) \right| = \infty, \text{ falls} p_i \not= n_1,...,n_m $ \\
		    Dieses $p_i$ ist eine PS $s_\infty$ der ÜF
             \end{itemize}
...
\[  	\text{mit } 
	\left\{ \begin{matrix}
	    ...
	    D_i = \frac{-\sigma_i}{\left| p_i \right|^2}, \left| D_i \right|^2 \text{ = Dämpfung}\\
	\end{matrix}\right. 
  \]
\item kleines/ großes $\frac{T_i}{D_i} = \frac{1}{\sigma_i} > 0 \bb{14} \to $ kleine/ große Abklingzeit der Einhüllenden 
\item großes/ kleines $D_i \to $ geringe/ hohe Schwingneigung (siehe \bb{7})
...
\item Damit resultiert als ÜF nach dem Ausklammern der Pole und Nullstellen $\not= 0$: 
\[G(s) = K \frac{(1-T_{zj}s)...(1+D_{z\mu}T_{z\mu}s + T_{z\mu}^2s^2)}
	  {s^q (1+T_1s)(1+t_2s)...(1 + 2D_\nu T_{\nu s} s + T_\nu^2 s^2)}
\]
mit $K = k \frac{\prod_n (n_j)}{\prod_{p_i \not= 0} (-p_i)}$ : Verstärkungsfaktor der Strecke (meist Annahme: $K>0$)
...
\item E/A Stabilität q=0 und $T_\nu D_\nu > 0  \forall \nu$

\end{itemize}

\section{Betrachtung des Strecken-Frequenzgang}
\subsection{Definition:}
Der Frequenzgang (FG) eines LZI-Glieds ist dessen ÜF G(s) über der $j\omega$ Achse, d.h. \\
$G(s) \xrightarrow[s=j\omega]{} G(j\omega)$ \\
Zeitkonstantenform der ÜF: $\xrightarrow[]{s=j\omega} G(j\omega) = K \frac{(1+T_{z1} j\omega)...}{(j\omega)^2 (1+T_1j\omega)...}$ \\
Beispiel: $PT_1$-Glied:\\
...
\subsection{Bedeutung des FG für das System-Zeitverhalten}
